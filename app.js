const queryUrl = "https://ron-swanson-quotes.herokuapp.com/v2/quotes/20";
    ...
    //Search for long quotes (5 or more words)
        ...
        $.ajax({
          ...
          for (let i = 0; i < response.length; i++) {
            ...
            if (quoteArr.length >= 5) {
              ...
              const longQuote = $(`<h3 class="rate" data-quote="quote-${[i]}">${quoteArr.join(" ")}</h3>`);
        ...
        $.ajax({
          ...
          for (let i = 0; i < response.length; i++) {
            ...
            if (quoteArr.length <= 4) {
              ...
              const shortQuote = $(`<h3 class="rate" data-quote="quote-${[i]}">${quoteArr.join(" ")}</h3>`);
      ...
      $(document).on("click", ".rate", function() {
        ...
        prompt(`On a scale of 1 to 5, what do you think of this Quote? ... "${$(this).text()}"`);
      });